    ...
    <nav className="navbar sticky top-0 z-30 bg-white/80 dark:bg-gray-950/80 backdrop-blur border-b border-gray-200 dark:border-gray-800 shadow-sm transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <Link href="/" className="flex items-center group">
            <span className="text-2xl font-extrabold tracking-tight text-gray-900 dark:text-white select-none">ClashCompare</span>
          </Link>
          {/* Desktop Nav */}
          <div className="hidden md:flex items-center gap-2">
            <NavLink href="/leaderboard/cards">Cards Leaderboard</NavLink>
            <NavLink href="/leaderboard/emotes">Emotes Leaderboard</NavLink>
      ...
      <div className={`md:hidden fixed top-0 right-0 w-64 h-full z-50 bg-white dark:bg-gray-950 shadow-lg transform transition-transform duration-300 ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col gap-2 p-6 pt-16">
          <NavLink href="/leaderboard/cards" onClick={() => setMenuOpen(false)}>Cards Leaderboard</NavLink>
          <NavLink href="/leaderboard/emotes" onClick={() => setMenuOpen(false)}>Emotes Leaderboard</NavLink>
          <NavLink href="/auth" onClick={() => setMenuOpen(false)}>Login</NavLink>
        </div>
      </div>
    </nav>
  )
...
function NavLink({ href, children, onClick }: { href: string, children: React.ReactNode, onClick?: () => void }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="relative px-4 py-2 rounded font-medium text-gray-700 dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400 transition-colors focus:outline-none group"
    >
      <span className="relative z-10">{children}</span>
      <span className="absolute left-4 right-4 -bottom-1 h-0.5 bg-gradient-to-r from-blue-500 to-purple-500 scale-x-0 group-hover:scale-x-100 transition-transform origin-left rounded-full" />
    </Link>
  )
}
